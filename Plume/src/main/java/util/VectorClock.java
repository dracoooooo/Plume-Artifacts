...
public class VectorClock implements Clock<VectorClock> {

	private int dim;
	private short tid;
	private Vector<Integer> clock;

	public VectorClock(short tid, int dim) {
		this.dim = dim;
		this.tid = tid;
	...
	@Override
	public void join(VectorClock clock) {
		updateMax2(clock);
	}

	@Override
	public void incrementBy(int val) {
		int newVal = this.clock.get(this.tid) + val;
		this.clock.set(this.tid, newVal);
	}
